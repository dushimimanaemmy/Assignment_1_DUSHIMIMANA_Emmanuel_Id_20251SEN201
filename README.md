...
### Query: SELECT o.order_id, c.customer_name, c.city, o.order_date FROM orders o 
...
### Query is: SELECT oi.order_item_id, p.product_name, p.category, p.price, oi.quantity 
...
### QUERY IS: SELECT c.customer_id, c.customer_name, c.city, o.order_id, o.order_date 
...
### QUERY IS: SELECT c.customer_id, c.customer_name, SUM(oi.quantity * p.price) AS total_spent, RANK() OVER ( ORDER BY SUM(oi.quantity * p.price) DESC ) AS spending_rank
...
### QUERY IS: SELECT o.order_id, c.customer_name, o.order_date, ROW_NUMBER() OVER ( PARTITION BY c.customer_id ORDER BY o.order_date )
...
### QUERY IS: SELECT o.order_date, o.order_id, SUM(oi.quantity * p.price) 
...
### QUERY IS: SELECT customer_name, order_id, order_date, previous_order_date, order_date - previous_order_date AS days_between_orders
FROM (
